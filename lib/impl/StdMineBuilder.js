        ...
        for (let ao in load.results[i].expr) {
          if (
            ...
            if (!Array.isArray(load.results[i].expr[ao])) {
              throw new Error(format(t('Syntax error in aggregation expression %s'), JSON.stringify(load.results[i])));
            }
      ...
      .then(
        (info) => {
          ...
          if (!Array.isArray(source.load) || !source.load.length) {
            throw new Error(t('Data source forming logic not specified.'));
          }
