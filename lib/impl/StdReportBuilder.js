...
const{ FunctionCodes: F, data: { Item }, PropertyTypes } = require('@iondv/meta-model-contracts');
...
function MongoReportBuilder(cOptions) {
  ...
  function buildOptions(mine, sheet, options, collectors) {
    ...
    if (options) {
      if (options.filter) {
        result.filter = result.filter ? {[F.AND]: [result.filter, options.filter]} : options.filter;
      }
      if (options.offset) {
    ...
    return cOptions.dataSource.aggregate(dstn(mine, sheet, options), opts)
      .then((data) => {
        ...
        collectors.forEach((collector) => {
          ...
          if (ids.length) {
            let filter = {[F.IN]: ['$' + collector.rkey, ids]};
            p = p.then(() => {
              let sh = {
                source: collector.source,
                fetch: collector.fetch,
                filter: collector.filter ? {[F.AND]: [filter, collector.filter]} : filter,
                sort: collector.sort
              };
    ...
    return p.then(() => {
      ...
      if (sheet.className) {
        ...
        if (options && options.filter) {
          f = f ? {[F.AND]: [f, options.filter]} : options.filter;
        }
